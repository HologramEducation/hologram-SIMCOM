...
bool HologramSIMCOM::begin(const int baud) {
  ...
  while(1) {
    ...
    if(_writeCommand("AT+CFUN=1\r\n", 10, "OK", "ERROR") != 2) {
      Serial.println(F("ERROR: begin() failed at +CFUN"));
      break;
    }
    ...
    if(_writeCommand("AT+CIPMUX=1\r\n", 1, "OK", "ERROR") != 2) {
      Serial.println(F("ERROR: begin() failed at +CIPMUX"));
      break;
    }

    // Set APN
    if(_writeCommand("AT+CSTT=\"hologram\"\r\n", 1, "OK", "ERROR") != 2) {
      Serial.println(F("ERROR: begin() failed at +CSTT"));
      break;
    }
